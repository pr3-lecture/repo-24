...
class AboutStrings < Neo::Koan
  ...
  def test_use_double_quotes_to_create_strings_with_single_quotes
    string = "Don't" #What a funny world that would be, where we wouldn't have apostrophes!
...
It was the worst of times.
  ...
  def test_here_documents_can_also_handle_multiple_lines #Careful with EOS ...
    long_string = <<EOS
It was the best of times,
It was the worst of times.
EOS
  ...
  def test_plus_will_concatenate_two_strings #As expected
    string = "Hello, " + "World"
    assert_equal "Hello, World", string
  end

  def test_plus_concatenation_will_leave_the_original_strings_unmodified #As expected
    hi = "Hello, "
    there = "World"
    string = hi + there
  ...
  def test_plus_equals_will_concatenate_to_the_end_of_a_string #As expected
    hi = "Hello, "
    there = "World"
    hi += there
  ...
  def test_plus_equals_also_will_leave_the_original_string_unmodified #As expected
  ...
  def test_the_shovel_operator_modifies_the_original_string
    ...
    there = "World"
    hi << there
    assert_equal "Hello, World", original_string # << operator changes original! + does not (see above)

    # THINK ABOUT IT:
    #
    # Ruby programmers tend to favor the shovel operator (<<) over the
    # plus equals operator (+=) when building up strings.  Why?
    # Everything flows into one var.
  end
  ...
  def test_single_quotes_sometimes_interpret_escape_characters
    string = '\\\''
    assert_equal 2, string.size
    assert_equal '\\\'', string #Conclusion, use double quotes when escaping?
  end
  ...
  def test_any_ruby_expression_may_be_interpolated
    ...
    sqString = Math.sqrt(5)
    assert_equal "The square root of 5 is #{sqString}", string
  end
