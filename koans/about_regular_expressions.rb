...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_a_regexp_can_search_a_string_for_matching_content #!Returns the match
  ...
  def test_question_mark_means_optional
    ...
    assert_equal "a", "abbcccddddeeeee"[/az?/] #Whoa ...already reluctant!
  end
  ...
  def test_asterisk_means_zero_or_more #Asterix returns an empty string and NOT nil
    ...
    #-> If it is to match a digit instead of a character or string
  end
  ...
  def test_the_left_most_match_wins #Greedy means it takes everything and only gives letters back if the regex threatens not to match.
    assert_equal "a", "abbccc az"[/az*/] #Hence it returning the very first match
  end
  ...
  def test_variables_can_also_be_used_to_access_captures
    assert_equal "Gray, James", "Name:  Gray, James"[/(\w+), (\w+)/] #Why does it not match to "Name:" ???
    assert_equal "Gray", $1
  ...
  end #TODO look into sub

  def test_gsub_is_like_find_and_replace_all
    assert_equal "one t-t", "one two-three".gsub(/(t\w*)/) { $1[0, 1] }
  end #TODO look into gsub
end
